select 
    ...
    ,ifnull(rec.erp_clustered_receivable_id,rec_v2.erp_clustered_receivable_id) as erp_clustered_receivable_id -- id do aglutinado ajustado para o caso de multa
    ,concat(ifnull(rec.erp_clustered_receivable_id,rec_v2.erp_clustered_receivable_id) ,".1") as erp_clustered_receivable_id_not_fiscal -- id do aglutinado ajustado para o caso de multa
    ,otc.front_id -- id do front
    ,otc.fin_id -- id do fin
    ,otc.conciliator_id -- id do conciliator_id
    ,otc.minifactu_id -- id do minifactu ajustado para o caso de multa
    ,concat(otc.minifactu_id,".1") as minifactu_id_not_fiscal -- id do minifactu ajustado para o caso de multa
    ...
    ,if(ivcr.type_person='foreign','7949',if(oftv.state=ivcr.state,'5933','6949')) as cfop_code
from invoice ivc
...
and iec.to_generate_fiscal_document = 'no'
...
and iit.to_generate_fiscal_document = 'no' -- Filtra somente os itens que não gera transmissão fiscal - para os casos de multa contratual
